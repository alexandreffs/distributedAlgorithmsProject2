...
import protocols.app.requests.CurrentSnapshotReply;
import protocols.app.requests.CurrentSnapshotRequest;
import protocols.app.requests.InstallSnapshotRequest;
...
public class HashApp extends GenericProtocol {
    ...
    public HashApp(Properties properties) throws HandlerRegistrationException, IOException {
        ...
        String address = properties.getProperty("babel.address");
        ...
        subscribeNotification(ClientRequestReply.NOTIFICATION_ID, this::uponClientRequestReply);

        /*-------------------- Register Request Handler ---------------------------- */
        registerRequestHandler(CurrentSnapshotRequest.REQUEST_ID, this::uponCurrentStateRequest);
        registerRequestHandler(InstallSnapshotRequest.REQUEST_ID, this::uponInstallStateRequest);
    ...
    private void uponCurrentStateRequest(CurrentSnapshotRequest req, short sourceProto) {
        byte[] state;
        try {
        ...
        sendReply(new CurrentSnapshotReply(req.getInstance(), state), sourceProto);
    }

    private void uponInstallStateRequest(InstallSnapshotRequest req, short sourceProto) {
        try {
    ...
    private void uponClientRequestReply(ClientRequestReply executeNotificat, short sourceProto) {
        try {
            //Deserialize operation received
            Operation op = Operation.fromByteArray(executeNotificat.getOperation());
            ...
            Pair<Host, Long> pair = clientIdMapper.remove(executeNotificat.getOpId());
            if (pair != null) {
