...
public class StateMachine extends GenericProtocol {
    ...
    /*
     * Operations already submitted to the agreement protocol by this SMR.
     *
     * This avoids proposing the same operation multiple times while it is still
     * pending. After a leader change, this set is cleared because a previously
     * submitted request may have been sent to an old leader and may never be
     * decided.
     */
    private final Set<UUID> submittedOperations;
    ...
    public StateMachine(Properties props) throws IOException, HandlerRegistrationException {
        ...
        this.submittedOperations = new HashSet<>();
    ...
    private void uponOrderRequest(OrderRequest request, short sourceProto) {
        ...
        if (executedOperations.contains(request.getOpId())) {
            logger.debug("Ignoring already executed client request {}", request.getOpId());
            return;
        }
    ...
    private void uponForwardRequestMessage(
        ...
        if (executedOperations.contains(msg.getOpId())) {
            logger.debug("Ignoring already executed forwarded request {}", msg.getOpId());
            return;
        }
    ...
    private void uponDecidedNotification(DecidedNotification notification, short sourceProto) {
        ...
        /*
         * Important:
         * Even if this replica was not leader before, it must learn which instances
         * already exist. If it becomes leader later, it must propose after the highest
         * known instance.
         */
        nextInstanceToPropose = Math.max(nextInstanceToPropose, instance + 1);
    ...
    private void uponLeaderChangeNotification(LeaderChangeNotification notification, short sourceProto) {
        ...
        /*
         * After a leader change, previous submissions may have been lost.
         * Allow pending local requests to be submitted again to the new leader.
         */
    ...
    private void submitOrForward(UUID opId, byte[] operation) {
        if (executedOperations.contains(opId)) {
            logger.debug("Not submitting already executed operation {}", opId);
            return;
        }
    ...
    private void proposeToAgreement(UUID opId, byte[] operation) {
        if (executedOperations.contains(opId)) {
            logger.debug("Not proposing already executed operation {}", opId);
            return;
        }

        if (submittedOperations.contains(opId)) {
            logger.debug("Not proposing already submitted operation {}", opId);
            return;
        }

        submittedOperations.add(opId);
    ...
    private void forwardToLeader(UUID opId, byte[] operation) {
        if (executedOperations.contains(opId)) {
            logger.debug("Not forwarding already executed operation {}", opId);
            return;
        }
    ...
    private void executeDecision(DecidedNotification decision) {
         ...
         */
        if (executedOperations.contains(opId)) {
            submittedOperations.remove(opId);
...
// LogManager.getLogger(StateMachine.class);
...
// public StateMachine(Properties props) throws IOException,
...
// registerChannelEventHandler(channelId, InConnectionUp.EVENT_ID,
...
// registerMessageSerializer(channelId, ForwardRequestMessage.MSG_ID,
// ForwardRequestMessage.serializer);
...
// subscribeNotification(DecidedNotification.NOTIFICATION_ID,
// this::uponDecidedNotification);
// subscribeNotification(LeaderChangeNotification.NOTIFICATION_ID,
// this::uponLeaderChangeNotification);
// }
...
// if (hostElements.length != 2) {
// throw new IllegalArgumentException("Invalid host in initial_membership: " +
// s);
// }
...
// } else {
// state = State.JOINING;
// logger.info("Starting in JOINING because I am not part of initial
// membership");
...
// if (state != State.ACTIVE) {
// logger.warn("Ignoring forwarded request while not ACTIVE: {}",
// msg.getOpId());
// return;
// }
...
// * to the current leader if we know one.
// */
// logger.debug("Received forwarded request but I am not leader. Current leader:
// {}", currentLeader);
// forwardToLeader(msg.getOpId(), msg.getOperation());
// }
// }
...
// private void uponDecidedNotification(DecidedNotification notification, short
// sourceProto) {
// logger.debug("Received DecidedNotification: {}", notification);
...
// if (instance < nextInstanceToExecute) {
// logger.debug("Ignoring old decision for already executed instance {}",
// instance);
// return;
// }

// DecidedNotification previous = decidedBuffer.putIfAbsent(instance,
// notification);

// if (previous != null) {
// logger.debug("Already had decision for instance {}. Ignoring duplicate.",
// instance);
// return;
// }
...
// private void uponLeaderChangeNotification(LeaderChangeNotification
// notification, short sourceProto) {
// Host newLeader = notification.getLeaderID();
...
// if (currentLeader != null && !currentLeader.equals(self) &&
// membership.contains(currentLeader)) {
// openConnection(currentLeader);
// }
...
// logger.info("Resubmitting {} pending local requests",
// pendingLocalRequests.size());

// /*
// * Copy the values to avoid concurrent modification if decisions arrive while
// * we are iterating.
// */
// List<PendingRequest> pending = new
// ArrayList<>(pendingLocalRequests.values());
...
// private void uponOutConnectionDown(OutConnectionDown event, int channelId) {
// logger.debug("Connection to {} is down, cause {}", event.getNode(),
// event.getCause());
// }

// private void uponOutConnectionFailed(OutConnectionFailed<ProtoMessage> event,
// int channelId) {
// logger.debug("Connection to {} failed, cause: {}", event.getNode(),
// event.getCause());
...
// private void uponInConnectionDown(InConnectionDown event, int channelId) {
// logger.trace("Connection from {} is down, cause: {}", event.getNode(),
// event.getCause());
// }
// }